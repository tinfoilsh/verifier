...
response=$(curl -s -D - "$BASE_URL_TDX/qe/identity")
...
curl -s "$BASE_URL_CERTS/IntelSGXRootCA.der" -o root_ca.crl

echo "Fetching PCK CRL (processor CA)..."
response=$(curl -s -D - "$BASE_URL_CERTS/pck-crl?ca=processor&encoding=der" -o pck_crl_processor.crl)
echo "$response" | grep -i "^Sgx-Pck-Crl-Issuer-Chain:" | sed 's/^[^:]*: //' | tr -d '\r' > pck_crl_processor_chain.txt

echo "Fetching PCK CRL (platform CA)..."
response=$(curl -s -D - "$BASE_URL_CERTS/pck-crl?ca=platform&encoding=der" -o pck_crl_platform.crl)
...
FMSPCS=(
    "90c06f000000"  # Azure/test TDX FMSPC
)

for fmspc in "${FMSPCS[@]}"; do
    echo "Fetching TCB Info for FMSPC $fmspc..."
    response=$(curl -s -D - "$BASE_URL_TDX/tcb?fmspc=$fmspc")
...
grep -o '"tcbEvaluationDataNumber":[0-9]*' qe_identity.json | head -1

for fmspc in "${FMSPCS[@]}"; do
    if [ -s "tcb_info_${fmspc}.json" ]; then
        echo "TCB Info ($fmspc) tcbEvaluationDataNumber:"
        grep -o '"tcbEvaluationDataNumber":[0-9]*' "tcb_info_${fmspc}.json" | head -1
    fi
done
