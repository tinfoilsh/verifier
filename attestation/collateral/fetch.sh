...
BASE_URL_SGX="https://api.trustedservices.intel.com/sgx/certification/v4"
...
response=$(curl -sf -D - "$BASE_URL_SGX/pckcrl?ca=processor&encoding=der" -o pck_crl_processor.crl)
echo "$response" | grep -i "^Sgx-Pck-Crl-Issuer-Chain:" | sed 's/^[^:]*: //' | tr -d '\r' > pck_crl_processor_chain.txt

echo "Fetching PCK CRL (platform CA)..."
response=$(curl -sf -D - "$BASE_URL_SGX/pckcrl?ca=platform&encoding=der" -o pck_crl_platform.crl)
