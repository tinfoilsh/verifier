package attestation

import (
	"testing"

	"github.com/stretchr/testify/assert"
)

func TestSevVerify(t *testing.T) {
	att := `{
	"format":"https://tinfoil.sh/predicate/sev-snp-guest/v1",
	"body":""
}`
	verification, err := VerifyAttestationJSON([]byte(att))
	assert.Nil(t, err)

	assert.Equal(t, "65083e5904c0273b644ad90e51e12a8d76fe07f2ab8ab14d76011b3e9c7dcaa7", verification.PublicKeyFP)
	assert.Equal(t, 1, verification.Measurement.RegisterCount())
	assert.Equal(t, "ff18f0a28cd150bb6123aa266adf07b4edac8ade942085f1b5283d37750ee924ac16199c2bfec7d42a6ab3964354685f", verification.Measurement.GetRegister(0))
}
